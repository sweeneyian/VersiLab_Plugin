...
public class NiDaqSimpleDemo {
    ...
    /*
    Pointer to doTask is a thread that communicates with NI board
    writeDigitalOut doesnt need to create and close new task each time
    split original NiDaqSimpleDemo writeDigitalOut methods into initTask writeDigitalOut and endTask
    and make Pointer doTask a static variable in the class that needs initialising...
    */
    ...
    public static void initTask()throws NiDaqException{
        doTask = daq.createTask("Task");
        daq.createDOChan(doTask, "cDAQ1Mod1/port0/line0:3", "", Nicaiu.DAQmx_Val_ChanForAllLines);
        daq.startTask(doTask);
    }
    public static void endTask()throws NiDaqException {
        daq.stopTask(doTask);
